...
static unsigned long scan_swap_map(struct swap_info_struct *si,
				   unsigned char usage)
{
	unsigned long start_offset = si->cluster_next;
	unsigned long offset;

	/* Not really necessary since the caller is holding swap_lock. */
	si->flags += SWP_SCANNING;

	for (offset = start_offset; offset != start_offset - 1; offset++) {
