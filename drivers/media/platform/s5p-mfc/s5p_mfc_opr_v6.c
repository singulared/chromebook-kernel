		...
		struct s5p_mfc_runtime_enc_params *runtime_p)
{
	...
	if (runtime_p->params_changed & (1 << MFC_ENC_FRAME_INSERTION)) {
		unsigned int reg = READL(mfc_regs->e_frame_insertion);
		reg &= ~0x3;
...
static int s5p_mfc_set_enc_params(struct s5p_mfc_ctx *ctx)
{
	...
	WRITEL(ctx->img_width - (p->crop_left_offset + p->crop_right_offset),
		mfc_regs->e_cropped_frame_width);
...
static int s5p_mfc_init_decode_v6(struct s5p_mfc_ctx *ctx)
{
	...
	/* When user wants to set a 'display_delay'
	 * It should first set 'display_delay_enable' to 1 */
...
static int s5p_mfc_alloc_enc_buffers_v6(struct s5p_mfc_ctx *ctx)
{
	...
	case S5P_MFC_CODEC_H264_ENC:
		...
		ctx->scratch_buf_size = ALIGN(ctx->scratch_buf_size,
				S5P_FIMV_SCRATCH_BUFFER_ALIGN_V6);
		bank1_size = ctx->scratch_buf_size + ctx->tmv_buffer_size +
