					...
					struct host_cmd_ds_command *resp)
{
	...
	/*
	 * Ideally, our use of async_subsc_evt_storage would be symmetric such
	 * that we could prove that the previous async call had completed before
	 * this one starts but this would require identifying where right point
	 * to relinguish the resource is and it isn't clear if there is such a
