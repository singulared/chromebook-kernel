...
int mwifiex_sta_init_cmd(struct mwifiex_private *priv, u8 first_sta)
{
	...
	if (first_sta) {
		...
		ret = mwifiex_send_cmd(priv,
					     ...
					     &priv->adapter->tx_buf_size, false);
		if (ret)
			return -1;
		...
		ret = mwifiex_send_cmd(priv,
					     ...
					     EN_AUTO_PS, BITMAP_STA_PS, NULL, false);
		if (ret)
			return -1;
	}
	...
	ret = mwifiex_send_cmd(priv, HostCmd_CMD_802_11_SNMP_MIB,
				     HostCmd_ACT_GEN_SET, DOT11D_I, &state_11d, false);
	if (ret)
	...
	ret = mwifiex_send_cmd(priv, HostCmd_CMD_11N_CFG,
				     HostCmd_ACT_GEN_SET, 0, &tx_cfg, false);
