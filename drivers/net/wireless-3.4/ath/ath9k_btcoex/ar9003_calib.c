				...
				bool longcal)
{
	 ...
	 */
	if (longcal && ath9k_btcoex_ath9k_hw_getnf(ah, chan)) {
		/*
...
static bool ar9003_hw_tx_iq_cal_run(struct ath_hw *ah)
{
	...
	if (!ath9k_btcoex_ath9k_hw_wait(ah, AR_PHY_TX_IQCAL_START,
...
skip_tx_iqcal:
	if (run_agc_cal || !(ah->ah_flags & AH_FASTCC)) {
		...
		status = ath9k_btcoex_ath9k_hw_wait(ah, AR_PHY_AGC_CONTROL,
				       AR_PHY_AGC_CONTROL_CAL,
				       0, AH_WAIT_TIMEOUT);
		if (AR_SREV_9462(ah)) {
			for (i = 0; i < AR9300_MAX_CHAINS; i++) {
