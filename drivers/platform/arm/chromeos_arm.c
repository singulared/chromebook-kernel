...
static int __devinit chromeos_arm_panic_init(struct platform_device *pdev,
					     u32 start, u32 size)
{
	int ret = -EINVAL;
	struct chromeos_arm_elog_panic_buffer *buf;
...
static int __devexit chromeos_arm_remove(struct platform_device *pdev)
{
	struct chromeos_arm_elog_panic_buffer *buf;
...
static struct platform_driver chromeos_arm_driver = {
	...
	.remove = __devexit_p(chromeos_arm_remove),
	.driver = {
		.name = "chromeos_arm",
	},
};
