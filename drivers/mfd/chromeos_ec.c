...
struct chromeos_ec_device *cros_ec_alloc(const char *name)
{
...
static int check_protocol_version(struct chromeos_ec_device *ec)
{
	int ret;
...
int cros_ec_register(struct chromeos_ec_device *ec_dev)
{
	...
	for_each_child_of_node(dev->of_node, node) {
		...
		err = mfd_add_devices(dev, 0x10, &cell, 1, NULL, 0, NULL);
		if (err)
...
int cros_ec_remove(struct chromeos_ec_device *ec_dev)
{
