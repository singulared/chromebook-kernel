				    ...
				    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = ec_data.current_system * 1000;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = ec_data.voltage_system * 1000;
		break;
...
static struct platform_driver cros_ec_charger_driver = {
	.driver = {
		.name = "cros_ec-charger",
		.owner = THIS_MODULE,
		.pm = &cros_ec_charger_pm_ops,
	},
	.probe = cros_ec_charger_probe,
	.remove = __devexit_p(cros_ec_charger_remove),
};

module_platform_driver(cros_ec_charger_driver);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("ChromeOS EC charger");
MODULE_ALIAS("platform:cros_ec-charger");
